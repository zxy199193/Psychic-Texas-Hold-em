...
public class PokerUIManager : MonoBehaviour
{
    ...
    public ShockwaveController shockwave; // 注册冲击波指挥官
    ...
    private Dictionary<Button, int> activeDynamicSkillButtons = new Dictionary<Button, int>();
    ...
    private void GenerateInGameSkillBar()
    {
        ...
        foreach (int equippedID in localSelectedSkills)
        {
            ...
            if (btn != null)
            {
                ...
                // 把刚生成的按钮和它的耗蓝存进账本！
                activeDynamicSkillButtons.Add(btn, config.energyCost);
            }
        }
    }
    ...
    public void RefreshSkillButtonsState(int currentEnergy)
    {
        ...
        {
            if (kvp.Key != null) kvp.Key.interactable = (currentEnergy >= kvp.Value);
        }
        ...
        // 3. 处理固定技能：抵抗 (Resist)
        // 抵抗按钮通常由 ShowCastBar 控制，这里建议不要手动修改它的 interactable
        // 除非你想做“能量不足无法抵抗”的效果
    }
    private void Update()
    {
        ...
        if (PokerPlayer.LocalPlayer != null)
        {
            ...
            if (turnStatusText != null && !isShowingResult)
            {
                ...
                else if (string.IsNullOrEmpty(currentActingPlayerName))
                {
                    ...
                    // 动态读取服务器当前的状态，精准区分结算阶段
                    if (ServerGameManager.Instance != null)
                    {
                        string phaseStr = ServerGameManager.Instance.currentPhase.ToString();
                        // 只要服务器阶段包含这些代表“结算”的关键词（不管你后台定义的枚举叫 Showdown 还是 Settlement）
                        if (phaseStr.Contains("Showdown") || phaseStr.Contains("Settle") || phaseStr.Contains("End") || phaseStr.Contains("Award"))
                        {
                            statusMsg = "结算中...";
                        }
                    }
    ...
    public void RevealCommunityCards(int startIndex, int count, Card[] cards)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            if (startIndex + i < communityArea.childCount)
            {
                ...
                // 【修改这里】：把原来的 SetCard 换成华丽的 FlipToFace！
                // 并且通过 DOVirtual.DelayedCall 让多张牌依次翻开，不要同时翻，超帅！
                int index = i; // 闭包陷阱防护
